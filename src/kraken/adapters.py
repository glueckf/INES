...
def build_eval_plan(
    nw: List[Any],
    selectivities: Dict[str, Any],
    my_plan: List[Any],
    central_plan: List[Any],
    workload: List[Any],
) -> io.StringIO:
    """
    ...
    try:
        ...
        if not isinstance(my_plan, list) or len(my_plan) != 3:
            raise ValueError(
                f"my_plan must be a list of length 3, got {type(my_plan)} with length {len(my_plan) if isinstance(my_plan, list) else 'N/A'}"
            )

        if not isinstance(central_plan, list) or len(central_plan) != 3:
            raise ValueError(
                f"central_plan must be a list of length 3, got {type(central_plan)} with length {len(central_plan) if isinstance(central_plan, list) else 'N/A'}"
            )
        ...
        logger.debug(
            f"build_eval_plan called with nw={len(nw)} nodes, workload={len(workload)} items"
        )
        ...
        if not isinstance(result, io.StringIO):
            logger.warning(
                f"generate_eval_plan returned unexpected type: {type(result)}"
            )
