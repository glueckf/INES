...
def print_kraken(integrated_operator_placement_results):
    ...
    formatted_results = integrated_operator_placement_results.get(
        "formatted_results", {}
    )
    ...
    logger.info(
        f"   Execution Time: {metadata.get('execution_time_seconds', 0):.2f} seconds"
    )
    ...
    logger.info(
        f"   Central Hop Latency: {metadata.get('central_hop_latency', 0)} hops"
    )
    ...
    for projection, decision in placement_decisions.items():
        if hasattr(decision, "node") and hasattr(decision, "costs"):
            ...
            logger.info(
                f"      Resources Sufficient: {decision.has_sufficient_resources}"
            )
            ...
            if hasattr(decision, "plan_details") and decision.plan_details:
                ...
                logger.info(
                    f"      Computing Time: {details.get('computing_time', 0):.4f}s"
                )
                logger.info(f"      Latency: {details.get('latency', 0)} hops")
                logger.info(
                    f"      Transmission Ratio: {details.get('transmission_ratio', 0):.6f}"
                )
            # Empty line
