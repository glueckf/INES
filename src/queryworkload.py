...
def generate_workload(num_of_queries: int, query_length: list, primitive_events_data: list):  #count, lenthh
    qwl = []
    ...
    while len(qwl) != num_of_queries:
        mylength = int(rd.uniform(query_length / 2, query_length + 1))
        while mylength == 2:
            mylength = int(rd.uniform(2, query_length + 1))
        nesting_depth = int(rd.uniform(1, mylength - 1))
        x = rd.uniform(0, 1)
        if x <= 0.5:
            query = SEQ()
        else:
            query = AND()
        query.children = generateQ(query, nesting_depth, mylength, primitiveEvents, prim)
...
def hasdoubles(query):
    ...
    return len(prims) > len(list(set(prims)))
...
def generateQ(query, nestingdepth, mylength, PrimitiveEvent, Prim):
    count = 0
    children = []
    remainingPrims = (mylength - 1 - nestingdepth)
    if nestingdepth == 1:
        ...
        if isinstance(query, AND):
            ...
            myquery = AND()
        myquery.children = generateQ(myquery, nestingdepth - 1, mylength - count, PrimitiveEvent, Prim)
