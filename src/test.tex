...
\section{State Space Formulation}\label{sec:state_space_formulation}
    ...
    \subsection{State Transition Function}
        ...
        For each candidate node $n$, the function invokes \texttt{calculate\_costs} (Algorithm~\ref{alg:calculate_costs}) to generate all valid communication strategies $R_{\text{strat}}$ (both all-push and optimized push-pull), returned as a set of fully-costed Kraken vertices. It creates a new state $s_{\text{next}}$ for each vertex $v \in R_{\text{strat}}$, calculates its critical path latency, and prunes it if $L_{\text{critical}} > \Lambda_{\text{max}}$. The function returns all surviving successor states, sorted by their objective score, ready for the search algorithm.
        ...
        \begin{algorithm}[h]
            ...
            \ForEach{$n \in N_{\text{sorted}}$}{
              $R_{\text{strat}} \gets \texttt{calculate\_costs}(p, n, s_i)$ \tcp*[r]{Alg~\ref{alg:calculate_costs}, returns set of $v$}
              \ForEach{$v \in R_{\text{strat}}$}{
                $s_{\text{next}} \gets \texttt{create\_next\_candidate}(s_i, v)$\;
...
\section{Unified Cost Evaluation Framework}\label{sec:unified_cost_framework}
    ...
    The \texttt{calculate\_costs} function computes the metrics for both plans independently, applies our state-aware corrections (detailed in Section~\ref{sec:local_event_correction} and in Section~\ref{sec:sink_cost_adjustment}), and returns a set of Kraken vertices $R_{strat}$. If the PrePP algorithm determines that all-push is optimal, both plans $\hat{p}_{push}$ and $\hat{p}_{PrePP}$ are identical and $R_{strat}$ will contain only one unique vertex.

    The \texttt{expand} function then creates a new successor state $s_{next}$ for each unique vertex in $R_{strat}$. This approach allows the search algorithm to simultaneously explore placement decisions and communication strategies as distinct branches in the solution space.
    ...
    \begin{algorithm}[h]
        ...
        \ForEach{$\hat{p} \in \hat{P}$}{
            ...
            $c_{raw},\ell^t_{raw} \gets raw\_costs(\hat{p})$ \tcp*[r]{Sec~\ref{sec:foundational_transmission_cost_model} \& ~\ref{sec:latency_modelling}}
            $\ell^p_v \gets processing\_latency(\hat{p})$ \tcp*[r]{Section~\ref{sec:latency_modelling}}
            \texttt{\\}
            ...
            \texttt{\\}
            \tcp{4. Create vertex and attach metrics}
            $v_{\text{steps}} \gets \hat{p}_{\text{steps}}$ \tcp*[r]{Extract steps from $\hat{p} = (n, \hat{p}_{\text{steps}})$}
            $v \gets (p, n, v_{\text{steps}})$ \tcp*[r]{Create Kraken Vertex (Def~\ref{def:kraken_vertex})}
            $v.cost \gets c^{\text{final}}$ \tcp*[r]{Attach metrics to vertex object}
            $v.latency \gets \ell^{\text{final}}$
            $v.proc\_latency \gets \ell^p_v$
            $R_{strat} \gets R_{strat} \cup \{v\}$\;
        }
        \texttt{\\}
        \KwRet{$R_{strat}$}\;
    \end{algorithm}
    ...
    \subsection{Foundational Transmission Cost Model}\label{sec:foundational_transmission_cost_model}
        ...
        \subsubsection{Pull Request Cost ($C_{req}$)}
            ...
            This is the cost of sending filtering predicates (from streams in pull set $\Psi_i$) from $n$ to all source nodes $n_s \in \bigcup_{S \in \rho_i}\phi^{-1}(S)$ that produce one of the required streams $S \in \rho_i$.
    ...
    \subsection{Latency Modeling}\label{sec:latency_modelling}
        ...
        We address this by separating the concerns. The optimization search (Section~\ref{sec:search_strategies}) is guided \emph{only} by \emph{transmission latency} ($L^t_s$) as this is the dominant factor in distributed \ac{CEP}. We model and aggregate \emph{processing latency} ($L^p_s$) separately for post-optimization analysis, allowing us to assess the computational feasibility of a solution at each node.

        This separation is formalized by adapting the INEv latency model, which defines a vertex's latency as:

        \begin{equation}\label{eq:latency_model}
            \ell_v = \xi \cdot \ell^p_v + \ell^t_v +\max_{u \in \delta(v)}l(u)
        \end{equation}

        Our framework effectively sets the processing latency factor $\xi = 0$ during optimization, excluding the processing latency from the latency evaluation. This design is extensible: by setting $\xi = 1$ and ensuring metrics are dimensionally aligned (e.g., converting hops to milliseconds), the framework could easily be adapted to optimize for a single, unified latency metric.
        ...
        \subsubsection{Example: Smart Factory Transmission Latency.}
            ...
            We apply this model to the push-pull plan from our smart factory example. The plan consists of two steps: $a_1$ (Push P) and $a_2$ (Pull H). The latency for $a_1$ involves only the response: $\ell^t(a_1) = \ell^t_{res}(a_1) = \max(\text{dist}(n_6^s, n_0^c)) = 2$ hops. The latency for $a_2$ involves both request and response: $\ell^t(a_2) = \ell^t_{req}(a_2) + \ell^t_{res}(a_2) = \max(\text{dist}(n_0^c, n_4^s), \text{dist}(n_0^c, n_5^s)) + \max(\text{dist}(n_4^s, n_0^c), \text{dist}(n_5^s, n_0^c)) = 2 + 2 = 4$ hops. The total vertex latency is $\ell^t_v = \ell^t(a_1) + \ell^t(a_2) = 2 + 4 = 6$ hops, matching the motivating example (Section~\ref{sec:motivating_example}).
        ...
        \subsubsection{Example: Smart Factory Processing Latency.}
            ...
            In our smart factory example, the baseline processing latency for evaluating $p = \text{SEQ}(P,H)$ at $n_0^c$ (all-push) is proportional to its output rate $r^{out}(p) = 1800$. For our optimized PrePP plan $\hat{p}_{\text{PrePP}}$, the input event volume is $10$ (for P) + $90$ (from $H_1$) + $90$ (from $H_2$) = $190$ events. The baseline volume is $10 + 900 + 900 = 1810$ events. This gives a reduction ratio $ratio(\hat{p}_v) = 190 / 1810 \approx 0.105$. The new processing latency is $\ell^p_v = r^{out}(p) \cdot ratio(\hat{p}_v) = 1800 \cdot 0.105 = 189$. This optimization reduces processing latency (proportional to $190$ events) by 9.5x, while it reduces transmission cost (420 units) by 8.6x. The processing latency scales directly with the number of input events processed, whereas the transmission cost scales with those events multiplied by their network distance, explaining the different reduction factors.

    \subsection{Local Event Correction (C1.1)}\label{sec:local_event_correction}
    ...
    \subsection{Sink Cost Adjustment (C1.2)}\label{sec:sink_cost_adjustment}
        ...
        After adjusting for locality, we perform a second adjustment to the metrics, an adjustment absent in the original \ac{INEv}, which is critical for our hierarchical fog-cloud environment. In this setting, the evaluation of a final query (e.g., at a fog node) and consumption (at the cloud) may occur at different locations.

        The transmission costs need to account for delivering the final query results to the designated cloud sinks $N_c$. This adjustment is applied to the intermediate metrics (e.g., $c_v'$) of \emph{both} plans, i.e., $\hat{p}_{\text{push}}$ and $\hat{p}_{\text{PrePP}}$.

        In contrast, \ac{INEv} optimizes the sink placement so that the evaluation node is the final destination~\cite{akili_inev_2023, akili_muse_2021, purtzel_predicate-based_2022}. Consequently, INEv does not check the output rate of the root projections, which leads to fan-out if such a root projection $p \in Q$ is placed at a non-sink node $n \notin N_c$. The sink cost is the cost to transmit the final result of the root projection, i.e., the rate $r^{out}(p)$, from $n$ to all sinks:
    ...
    \subsection{State-Level Metric Aggregation}\label{sec:state_level_aggregation}
        ...
        When a new vertex $v$ (which encapsulates a specific plan via its $v_{\text{steps}}$ component) is added to a base state $s_{\text{base}}$, it produces a new derived state $s_{\text{next}}$. The aggregate metrics for $s_{\text{next}}$ are computed incrementally based on those of $s_{\text{base}}$ as follows:
        ...
        \begin{itemize}
            ...
            \begin{equation}\label{eq:state_cost_aggregation}
                C(s_{\text{next}}) = C(s_{\text{base}}) + v.cost
            \end{equation}
            ...
            \begin{equation}\label{eq:state_processing_latency}
                L^p(s_{\text{next}}) = L^p(s_{\text{base}}) + v.proc\_latency
            \end{equation}
        ...
        \begin{algorithm}[h]
            ...
            \For{$p \in \mathcal{O}^P_Q$}{
              \If{$p \in \text{dom}(\mathcal{V}_s)$}{
                ...
                $L[p] \gets v.latency + \ell_{\text{pred}}$ \tcp*[r]{Eq.~\ref{eq:latency_model} with $\xi=0$}
...
\section{Decoupled Search Strategies}\label{sec:search_strategies}
    ...
    \subsection{Beam-$k$ Search}\label{sec:beam_search}
...
\section{Correctness, Complexity and Limitations}\label{sec:correctness}
    ...
    We conclude this chapter by analyzing the theoretical properties of the Kraken framework. We address the correctness of our joint optimization model by justifying our integration of external components. We then analyze the computational complexity that necessitates our heuristic approach and, finally, discuss the limitations inherent in our design.
    ...
    \subsection{Correctness and Design Justification}
        ...
        \begin{figure}[h]
            \centering
            \includegraphics[width=0.5\linewidth]{img/chapter4/inev_split.png}
            \caption{INEv \emph{split} function time share}
            \label{fig:inev_split_function}
        \end{figure}

        Second, integrating push-pull logic directly into the split function is \textbf{computationally infeasible}. The combination generation algorithm is already a significant performance bottleneck, consuming a majority of the pre-processing time in our experiments, as shown in Figure~\ref{fig:inev_split_function}. Expanding this phase to consider all possible PrePP plans for every potential sub-projection would increase the computational cost exponentially, rendering the problem intractable for realistic workloads.

        Our framework validly substitutes \ac{INEv}'s original input cost model with the PrePP plan generator. The original \ac{INEv} placement algorithm treated input acquisition as a simple cost calculation. Our Kraken vertex (Definition~\ref{def:kraken_vertex}) replaces this with a call to the PrePP framework. Since PrePP finds the minimal-cost input plan for a given (projection, node) pair, it integrates cleanly into our state-space model, providing a far more accurate cost for the \emph{expand} function to evaluate.
