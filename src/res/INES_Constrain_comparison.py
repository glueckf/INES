...
def main():
    ...
    parser.add_argument(
        "--MNP", nargs="+", required=True, help="CSV files without constraints"
    )
    parser.add_argument(
        "--MNPHybrid", nargs="+", required=True, help="CSV files with constraints"
    )
    parser.add_argument(
        "-y",
        "--y_column",
        default="TransmissionRatio",
    ...
    if labels1 != labels2:
        raise ValueError(
            "The MaxParents values in both groups must be identical (based on filenames)."
        )
