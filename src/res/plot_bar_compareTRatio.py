...
def plot_bar(
    ...
    if legend_tags and len(legend_tags) != len(y_columns):
        raise ValueError(
            "The number of legend tags must match the number of y_columns."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot bar graph from CSV files with multiple Y values."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--legend_tags", nargs="+", help="Custom legend tags for each y-column"
    )
