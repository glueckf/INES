...
def main():
    ...
    if myargs.colors and len(myargs.colors) != len(myargs.inputs) * len(myargs.yC):
        print(
            "Number of colors must match the total number of input file and Y column combinations."
        )
        return

    if myargs.styles and len(myargs.styles) != len(myargs.inputs) * len(myargs.yC):
        print(
            "Number of styles must match the total number of input file and Y column combinations."
        )
        return

    if myargs.legend_labels and len(myargs.legend_labels) != len(myargs.inputs) * len(
        myargs.yC
    ):
        print(
            "Number of legend labels must match the total number of input file and Y column combinations."
        )
        return
    ...
    legend_labels = (
        myargs.legend_labels
        if myargs.legend_labels
        else [
            f"{labels[i]} - {y_col}" for i in range(len(labels)) for y_col in myargs.yC
        ]
    )
    Y_Columns = myargs.yC
...
def myparse_args(parser):
    parser.add_argument(
        "-i", "--inputs", nargs="+", help="Input CSV file paths", required=True
    )
    parser.add_argument(
        "-l", "--labels", nargs="+", help="Labels for input files", required=True
    )
    parser.add_argument("-x", "--xC", help="X-axis column name", required=True)
    parser.add_argument(
        "-y", "--yC", nargs="+", help="Y-axis column name(s)", required=True
    )
    parser.add_argument(
        "-c",
        "--colors",
        nargs="+",
        required=False,
        help="Colors for each (input file, Y column) combination",
    )
    parser.add_argument(
        "-s",
        "--styles",
        nargs="+",
        required=False,
        help="Line styles for each (input file, Y column) combination",
    )
    parser.add_argument(
        "-legend_labels",
        nargs="+",
        required=False,
        help="Custom labels for each line in the legend",
    )
    parser.add_argument(
        "-box",
        "--boxplot",
    ...
    parser.add_argument(
        "-o", "--outname", required=False, default="plot", help="Output file name"
    )
