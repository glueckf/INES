...
def getETBs(node, EventNodes, IndexEventNodes):
    ...
    return [list(IndexEventNodes.keys())[list(IndexEventNodes.values()).index(x)] for x in
            mylist]  # index from row id <-> etb
...
# TODO: Welche Funktionalitaet hat diese Funktion? Was sagt der Wert aus?
# Wir berechnen den Median, also den mittleren Abstand innerhalb des Netzwerkes, aber was sagt uns das aus?
# Welchen Sinn hat AllPairs und davon dann der Median von den durchschnittlichen Entfernungen,
# wenn wir einen gerichteten Graphen haben?
