...
class Tree:
    def __eq__(self, other):
        if isinstance(other, PrimEvent) and isinstance(self, PrimEvent):
            ...
            (isinstance(self, AND) and isinstance(other, AND))
            or (isinstance(self, SEQ) and isinstance(other, SEQ))
            or (isinstance(self, NSEQ) and isinstance(other, NSEQ))
        ):
            if len(self.children) != len(other.children):
                return False
            for i in self.children:
                mycount = self.children.count(i)
                if i not in other.children or other.children.count(i) != mycount:
                    return False
            for i in other.children:
                mycount = other.children.count(i)
                if i not in self.children or self.children.count(i) != mycount:
                    return False
            if (isinstance(self, SEQ) and isinstance(other, SEQ)) or (
                isinstance(self, NSEQ) and isinstance(other, NSEQ)
            ):
                truelist = [
                    1
                    for x in range(len(self.children))
                    if str(self.children[x]) == str(other.children[x])
                ]
                ...
                for childindex in range(len(i.children)):
                    ...
                    if isinstance(i.children[childindex], PrimEvent):
                        if i.children[childindex + 1 :]:
                            mysequence[str(i.children[childindex])].append(
                                list(
                                    map(
                                        lambda x: x.leafs(),
                                        i.children[childindex + 1 :],
                                    )
                                )
                            )
                    else:
                        for p in i.children[childindex].leafs():
                            if str(p) not in mysequence.keys():
                                mysequence[str(p)] = []

                            mysequence[str(p)].append(
                                list(
                                    map(
                                        lambda x: x.leafs(),
                                        i.children[childindex + 1 :],
    ...
    def ispartof(self, other):
        if set(list(map(lambda x: filter_numbers(x), self.leafs()))) != set(
            list(map(lambda x: filter_numbers(x), other.leafs()))
        ):
            return False
