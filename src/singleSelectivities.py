...
def initializeSingleSelectivity(CURRENT_SECTION, config_single, workload):
    ...
    for line in config_single:
        ...
        if CURRENT_SECTION == QUERIES:
            print(f"[SELECTIVITY] Processing query: {extract_queries(line)}")

        if CURRENT_SECTION == MUSE_GRAPH:
            print('[SELECTIVITY] Processing MUSE graph section...')
        ...
        if CURRENT_SECTION == SELECTIVITIES:
            # Debug: print(f"[SELECTIVITY] Processing line: {line}")
